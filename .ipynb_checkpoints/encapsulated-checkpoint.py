...
def time_change(imported_time):
    exported_time = imported_time.split('.')
    year = int(exported_time[0])
    month = int(exported_time[1])
    date = int(exported_time[2])
    exported_time = datetime.datetime(year, month, date)
    now = datetime.datetime.now()
    interval = datetime.timedelta(days = 7)

    if now.date() >= exported_time.date() >= (now - interval).date():
        print('it is in the interval')
    else:
        print('it is outside the interval')
    export_time = exported_time.date()
    return export_time
...
def make_certain_url():
    ...
    return search_urls, pages, search
...
def get_naver_news_title_and_content(headers, naver_url):
    ...
    news_record_time = time_change(news_record_time)
...
def main():
    ...
    # search_urls, pages, search = make_certain_url()
